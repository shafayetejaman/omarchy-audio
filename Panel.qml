...
Panel {
  ...
  // Port info from pactl: maps sink-name -> array of {name, label, isActive}
  property var portInfo: ({})
  property bool portInfoLoaded: false
  ...
  onPortInfoChanged: scheduleDisplayAudioModelRefresh()
  ...
  function refreshDisplayAudioModels() {
    ...
    displayOutputs = buildDisplayOutputs()
    clampCursor()
  }

  function buildDisplayOutputs() {
    var outputs = []
    var sinks = audioSinks
    for (var i = 0; i < sinks.length; i++) {
      var sink = sinks[i]
      var sinkName = String(sink.name || "")
      var ports = portInfoLoaded ? portInfo[sinkName] : null

      // If no port info or only one port, show the sink as-is
      if (!ports || ports.length <= 1) {
        outputs.push({
          node: sink,
          portName: ports && ports.length === 1 ? ports[0].name : "",
          portLabel: ports && ports.length === 1 ? ports[0].label : "",
          isActive: ports && ports.length === 1 ? ports[0].isActive : true,
          isCurrentOutput: root.sink && root.sink.id === sink.id
            && (!ports || ports.length === 0 || ports[0].isActive)
        })
      } else {
        // Multiple available ports: expand into separate entries
        for (var j = 0; j < ports.length; j++) {
          ...
          outputs.push({
            ...
            isCurrentOutput: root.sink && root.sink.id === sink.id && port.isActive
          })
        }
      }
    }
  ...
  Process {
    id: portInfoProc
    command: [Qt.resolvedUrl("port-list.sh").toString().replace("file://", "")]
    stdout: StdioCollector {
      waitForEnd: true
  ...
  component SinkRow: CursorSurface {
    ...
    readonly property var node: output.node
    readonly property string portName: output.portName || ""
    readonly property string portLabel: output.portLabel || ""
    readonly property bool isActive: output.isCurrentOutput
    ...
    Row {
      ...
      Text {
        text: sinkRow.portName ? Model.sinkGlyph(sinkRow.node) : root.sinkGlyph(sinkRow.node)
        color: root.bar.foreground
        font.family: root.bar.fontFamily
        font.pixelSize: Style.font.title
      ...
      Text {
        text: sinkRow.portLabel || root.nodeLabel(sinkRow.node)
        color: root.bar.foreground
        font.family: root.bar.fontFamily
